...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.cmenergy.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute('data-cmenergy-custom-color', changedInput.value);
    inputToSync.setAttribute('data-cmenergy-custom-color', changedInput.value);
  ...
  Drupal.behaviors.oliveroColorPicker = {
    attach: () => {
      ...
      const colorTextInputs = once(
        'cmenergy-color-picker',
        '[data-drupal-selector="cmenergy-color-picker"] input[type="text"]',
      );
